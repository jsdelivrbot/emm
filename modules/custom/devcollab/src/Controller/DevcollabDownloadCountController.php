...
namespace Drupal\devcollab\Controller;
...
*/
class DevcollabDownloadCountController extends ControllerBase {
...
//          ),
//            array(
//                'data' => $this->t('3Entity Type'),
//                'field' => 'type',
//            ),
            array(
                'data' => $this->t('Resource Link'),
                'field' => 'id',
            ),
            array(
        ...
        $view_all = '';
        if (\Drupal::currentUser()->hasPermission('view download counts')) {
            //$view_all = Link::fromTextAndUrl($this->t('View Details'), Url::fromRoute('download_count.details', array('download_count_entry' => 'all')))->toString();
            $view_all = t('View Details');
              $header[] = array(
                'data' => $view_all,
            );
        }
        $export_all = '';
        if (\Drupal::currentUser()->hasPermission('export download counts')) {
            //$export_all =  Link::fromTextAndUrl($this->t('Export CSV'), Url::fromRoute('download_count.export', array('download_count_entry' => 'all')))->toString();
            $export_all = t('Export CSV');
            $header[] = array(
                'data' => $export_all,
            );
        ...
        $reset_all='';
        if (\Drupal::currentUser()->hasPermission('reset download counts')) {
            //$reset_all =  Link::fromTextAndUrl($this->t('Reset Counts'), Url::fromRoute('download_count.reset', array('download_count_entry' => 'all')))->toString();
            $reset_all =  t('Reset Counts');
              $header[] = array(
                'data' => $reset_all,
            );
        ...
        foreach ($result as $file) {
            ...
            //$row[] = $file->id;
            $url = Url::fromRoute('entity.node.canonical', ['node' => $file->id]);
            $link_options = array(
              'attributes' => array(
                'target' => array(
                  '_blank',
            ...
            $row[] = Link::fromTextAndUrl($file->id, $url);
